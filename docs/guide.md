...
| **Менеджер магазина** | User с ролью `MANAGER` (или `is_staff` + группа) | CRUD категорий/товаров, скидки, настройки кэшбэка |

```text
Гость ──GET──► /api/categories/, /api/products/
Клиент ──JWT──► /api/cart/, /api/orders/, /api/newsletter/
Менеджер ──JWT──► /api/products/ (POST/PUT/DELETE), /api/discounts/, /api/settings/
```
...
| POST | `/checkout/preview/` | клиент | предпросмотр суммы |
...
Это ядро бизнеса. Вынесем в **сервис** `pricing.calculate_checkout(...)` (шаг 11), а не размажем по views.

```text
1. Сумма позиций = Σ (unit_price × qty)
   unit_price = price с учётом ProductDiscount (если активна)

2. Кэшбэк к списанию (если balance ≥ X и клиент запросил):
